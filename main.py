...
async def start(update, context):
    ...
    if not is_bot_active and user.id != ADMIN_ID: return
...
async def message_handler(update, context):
    ...
    if user_id == ADMIN_ID:
        if text == "⚙️ ✦ 𝔸𝕕𝕞𝕚𝕟 ℙ𝕒𝕟𝕖𝕝 ✦": await update.message.reply_text("Admin Panel:", reply_markup=admin_keyboard())
        ...
        elif text == "🔑 Add Keys": context.user_data["state"] = "select_game"; kb = [[g] for g in GAME_PLANS.keys()] + [["🔙 Back"]]; await update.message.reply_text("Select Game:", reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True))
        elif context.user_data.get("state") == "select_game":
            if text in GAME_PLANS: context.user_data["add_game"] = text; context.user_data["state"] = "select_plan"; kb = [[p] for p in GAME_PLANS[text].keys()] + [["🔙 Back"]]; await update.message.reply_text("Select Plan:", reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True))
        elif context.user_data.get("state") == "select_plan": context.user_data["add_plan"] = text; context.user_data["state"] = "add_keys"; await update.message.reply_text("Enter keys (one per line):", reply_markup=ReplyKeyboardMarkup([["🔙 Back"]], resize_keyboard=True))
            ...
            try:
                ...
                if keys:
                    msg = "\n".join([f"🎮 {g} ({p}): `{k}`" for g, p, k in keys]); context.user_data["resend_msg"] = msg; context.user_data["state"] = "confirm_resend"; context.user_data["target_uid"] = uid; await update.message.reply_text(f"Found:\n{msg}\n\nConfirm to resend?", reply_markup=ReplyKeyboardMarkup([["✅ Confirm Resend", "🔙 Back"]], resize_keyboard=True))
        ...
        elif context.user_data.get("state") == "confirm_resend" and text == "✅ Confirm Resend": await context.bot.send_message(context.user_data["target_uid"], f"🔄 *Your key has been resent:*\n\n{context.user_data['resend_msg']}", parse_mode="Markdown"); await update.message.reply_text("✅ Sent successfully!", reply_markup=admin_keyboard()); context.user_data.clear()
    ...
    if text == "🎮 ✦ 𝔾𝕒𝕞𝕖𝕤 ✦": kb = [[InlineKeyboardButton(g, callback_data=f"game_{g}")] for g in GAME_PLANS.keys()]; await update.message.reply_text("Select Game:", reply_markup=InlineKeyboardMarkup(kb))
        ...
        g = context.user_data.get("game", "N/A"); p = context.user_data.get("plan", "N/A"); btns = [[InlineKeyboardButton("✅ Accept", callback_data=f"acc_{user_id}_{g}_{p}"), InlineKeyboardButton("❌ Reject", callback_data=f"rej_{user_id}_{g}_{p}")]]; await context.bot.send_photo(ADMIN_ID, update.message.photo[-1].file_id, caption=f"Payment from {user_id}\nGame: {g}\nPlan: {p}", reply_markup=InlineKeyboardMarkup(btns)); await update.message.reply_text("✅ Screenshot sent!")

async def button_click(update, context):
    query = update.callback_query; await query.answer()
    if query.data.startswith("game_"):
        game = query.data.split("_")[1]; context.user_data["game"] = game; kb = [[InlineKeyboardButton(f"{p} - ₹{pr} ({get_stock_count(game, p)} Left)", callback_data=f"pay_{p}_{pr}")] for p, pr in GAME_PLANS[game].items()]; kb.append([InlineKeyboardButton("🔙 Back", callback_data="back_games")]); await query.edit_message_text(f"🎮 *{game}*\nSelect your plan:", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
    elif query.data == "back_games": kb = [[InlineKeyboardButton(g, callback_data=f"game_{g}")] for g in GAME_PLANS.keys()]; await query.edit_message_text("Select Game:", reply_markup=InlineKeyboardMarkup(kb))
    elif query.data.startswith("pay_"): data = query.data.split("_"); plan, price, game = data[1], data[2], context.user_data.get("game"); context.user_data["plan"] = plan; [await query.message.reply_photo(photo=open("qr.JPG", "rb"), caption=f"✅ *Plan:* {game} ({plan})\n💰 *Amount:* ₹{price}\n\n👉 Pay to this QR.", parse_mode="Markdown") for _ in [1] if os.path.exists("qr.JPG")] or await query.message.reply_text("⚠️ QR file not found!")
    elif query.data.startswith(("acc_", "rej_")):
        data = query.data.split("_"); action, uid, game, plan = data[0], int(data[1]), data[2], data[3]
        if action == "acc": key = approve_and_assign_key(uid, game, plan); [await context.bot.send_message(uid, f"🎉 *Key:* `{key}`") or await query.edit_message_caption(caption="✅ Approved!") for _ in [1] if key] or await query.edit_message_caption(caption="⚠️ No keys!")
        else: await context.bot.send_message(uid, "❌ Payment Rejected."); await query.edit_message_caption(caption="❌ Rejected!")
