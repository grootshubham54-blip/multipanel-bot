...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # एडमिन लॉजिक
    if user_id == ADMIN_ID:
        if text.startswith("Maintenance:"): is_bot_active = not is_bot_active; await update.message.reply_text(f"Maintenance: {'ON' if is_bot_active else 'OFF'}", reply_markup=admin_keyboard()); return
        ...
        elif context.user_data.get("state") == "select_plan": context.user_data["add_plan"] = text; context.user_data["state"] = "add_keys"; await update.message.reply_text("Enter keys:")
