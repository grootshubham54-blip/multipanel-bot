...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_ID:
        if text.startswith("Maintenance:"):
            ...
            await update.message.reply_text(f"Maintenance is now {'ON' if is_bot_active else 'OFF'}", reply_markup=admin_keyboard())
            return
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("game_"):
        ...
        parts = data.split("__"); game, plan, price = parts[1], parts[2], parts[3]; context.user_data["game"] = game; context.user_data["plan"] = plan
        try:
        ...
        parts = data.split("_", 3); uid, game, plan = int(parts[1]), parts[2], parts[3]; key = approve_and_assign_key(uid, game, plan)
